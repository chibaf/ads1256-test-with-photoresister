...
float sensor5, sensor6, sensor7, sensor8;
...
{ 
  ...
  adc.setChannel(0);   // Set the MUX for differential between ch2 and 3 
  ...
  adc.waitDRDY();
  adc.setChannel(3); // switch back to MUX AIN0 and AIN1
  sensor4 = adc.readCurrentChannel(); // DOUT arriving here are from MUX AIN6 and AIN7

  adc.waitDRDY();
  adc.setChannel(4); // switch back to MUX AIN0 and AIN1
  sensor5 = adc.readCurrentChannel(); // DOUT arriving here are from MUX AIN6 and AIN7
  adc.waitDRDY();
  adc.setChannel(5); // switch back to MUX AIN0 and AIN1
  sensor6 = adc.readCurrentChannel(); // DOUT arriving here are from MUX AIN6 and AIN7
  adc.waitDRDY();
  adc.setChannel(6); // switch back to MUX AIN0 and AIN1
  sensor7 = adc.readCurrentChannel(); // DOUT arriving here are from MUX AIN6 and AIN7
  adc.waitDRDY();
  adc.setChannel(7); // switch back to MUX AIN0 and AIN1
